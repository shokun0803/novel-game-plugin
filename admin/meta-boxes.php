...
function noveltool_ajax_create_scene() {
    ...
    // nonceチェック
    if ( ! wp_verify_nonce( $_POST['_ajax_nonce'], 'novel_game_meta_box_nonce' ) ) {
...
function noveltool_meta_box_callback( $post ) {
    ...
    <table class="form-table">
        <tr>
            <th scope="row">
                <label for="novel_game_title"><?php _e( 'ゲームタイトル', 'novel-game-plugin' ); ?></label>
            </th>
            <td>
                <input type="text" 
                       id="novel_game_title" 
        ...
        <tr>
            <th scope="row">
                <label for="novel_background_image"><?php _e( '背景画像', 'novel-game-plugin' ); ?></label>
            </th>
            <td>
                <input type="hidden" 
                ...
                <p>
                    <button type="button" 
                        ...
                        <?php _e( 'メディアから選択', 'novel-game-plugin' ); ?>
                    </button>
                </p>
        ...
        <tr>
            <th scope="row">
                <label for="novel_character_image"><?php _e( 'キャラクター画像', 'novel-game-plugin' ); ?></label>
            </th>
            <td>
                <input type="hidden" 
        ...
        <tr>
            <th scope="row">
                <label for="novel_dialogue_text"><?php _e( 'セリフ', 'novel-game-plugin' ); ?></label>
            </th>
            <td>
        ...
        <tr>
            <th scope="row">
                <label><?php _e( '選択肢', 'novel-game-plugin' ); ?></label>
            </th>
            <td>
                <div id="novel-choices-box">
                    <table id="novel-choices-table" class="widefat">
                        <thead>
                            <tr>
                                <th><?php _e( 'テキスト', 'novel-game-plugin' ); ?></th>
                                <th><?php _e( '次のシーン', 'novel-game-plugin' ); ?></th>
                                <th><?php _e( '操作', 'novel-game-plugin' ); ?></th>
                            </tr>
                        </thead>
                        <tbody>
                            <!-- JavaScriptで動的に生成 -->
                    ...
                    <p>
                        <button type="button" 
                                class="button" 
                                id="novel-choice-add">
                            <?php _e( '選択肢を追加', 'novel-game-plugin' ); ?>
                        </button>
                    </p>
                    <input type="hidden" 
...
function noveltool_save_meta_box_data( $post_id ) {
    ...
    if ( ! isset( $_POST['novel_game_meta_box_nonce'] ) || 
         ! wp_verify_nonce( $_POST['novel_game_meta_box_nonce'], 'novel_game_meta_box' ) ) {
        return;
    }
    ...
    foreach ( $fields as $field => $meta_key ) {
        if ( isset( $_POST[ $field ] ) ) {
            ...
            // サニタイズ処理
            if ( in_array( $field, array( 'dialogue_text', 'choices' ) ) ) {
