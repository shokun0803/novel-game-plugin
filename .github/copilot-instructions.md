...
### PRコメント投稿ルール（PRで `@copilot` を使う際の追加要件）
PR上で Copilot Agent に作業を依頼するためのコメントを投稿する際は、次の要件を満たしてください。これは Copilot Agent が正しく処理できるようにするための必須ルールです。

このセクションの参照指示（例: 「投稿時はこのセクションを守ること」）を受けた場合は、投稿者は本文に個別再掲がなくても本要件を**自動適用**してください。
特に `@copilot` の付与は任意ではなく、未付与・位置不正（先頭行以外）の場合は投稿を完了扱いにせず、修正してから投稿してください。

1. コメント冒頭（1行目）で必ず `@copilot` を**単独行**で明示してメンションすること（例: `@copilot`）。先頭行より前に空行・見出し・説明文を入れないでください。引用記法（`>`）・箇条書き（`-`）・コードブロック内での記載は無効として扱います。
2. コメント内で「パッチが作成できます」や「patch を作成します」といった文言を記載しないこと。Copilot Agent はメンションでのリクエストとして動作し、パッチ作成は Agent が必要と判断した場合に実施されます。
3. 問題の箇所は、**ファイル名**、関数名、行番号の目安（例: `admin/game-settings.php` の `noveltool_admin_post_delete_scene` の付近）を明確に指示してください。可能なら該当箇所の短いコードスニペット（コードブロック）を貼付してください。
4. 指示は項目ごとに整理してください（例: `1) 権限チェックの追加`、`2) UI 文言の修正` など）。各項目について、具体的な修正指示を漏れなく記載し、**必ずすべて実装すること**を前提に記述してください（「必須／推奨」等の順位付けはしない）。
5. 既に実装済みの修正や評価については詳細を記載しないでください（不要な冗長・重複を避けるため）。
6. 改行コードには通常の改行（\n）を使い、`\n\n` のようなエスケープ表記をコメント本文に含めないでください。
7. コメントに含める文字は GitHub の通常投稿で問題にならないものに限定し、不正な記号や非表示文字を使わないでください。
8. コードや変数名で `$` を含む場合、投稿上で欠落しないよう適切にエスケープ（コードブロックやエスケープ表現）してください。
 9. 長いコードやファイル全体を共有する場合は、短いスニペット以外はコードブロック（```php ... ```）を使用した上で、以下の安全な共有手順を推奨します。
10. 長いコマンドや長文のコメントを投稿する際の注意:
  - Terminal や VSCode の入力欄に長いコマンドや複数行のコードを「一つのコマンドとして」そのまま貼り付けると、誤ってターミナルで実行されたり、UI による改行やエスケープでエラーとなりコメント投稿が失敗する可能性があります。必ず回避してください。
  - 回避方法の一例（推奨）:
    1) まずローカルに `.md`（Markdown）ファイルを作成し、投稿したい長文・長いコマンド・スクリプトを保存します。
    2) エディタ上で `.md` ファイルを開き、コメント欄に**普通のテキストとして貼り付け**ます（貼り付けはコメント入力欄に対して行い、ターミナル内に貼らないでください）。
    3) 必要に応じて、該当部分はコードブロック（```bash または ```php 等）で囲んで投稿してください。
    4) 一時ファイルを作成した場合は、作業完了後に必ず削除してください。
  - 上記は、VSCode Copilot チャットや GitHub 上の入力欄で長いテキストを直接編集・送信する際の失敗を防ぐための方法です。
11. 過去に同じ箇所に対して指示・修正が繰り返されている場合、必ず過去の修正履歴と PR コメントを確認して、手戻りや同じ問題が再発しないように指示を作成してください。
12. コメント投稿が失敗した場合は VS Code ターミナル等から `gh` CLI を使ってコメントを投稿し、発生したエラー・出力を PR に添えて報告してください（なるべく GitHub Web UI での投稿を優先）。
13. コメント投稿時にエラーが発生した場合は、取得できたエラー内容を明示して報告してください。投稿は可能な限り通常の方法（GitHub Web UI での直接投稿）を優先してください。
14. 投稿内容にミスを発見した場合は新規の再投稿を行わず、既存コメントの編集で修正してください。二重投稿にならないよう厳守してください。

これらの要件は、Agent が正確に意図を把握し、コードを安全に修正・提案できるようにするためのものです。
