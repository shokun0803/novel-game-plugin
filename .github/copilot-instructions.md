...
8. コードや変数名で `$` を含む場合、投稿上で欠落しないよう適切にエスケープ（コードブロックやエスケープ表現）してください。
 ...
 10. 長いコードやファイル全体を共有する際の推奨方法（安全な共有手順）:
   - Gist を作成し、そのリンクをコメントで共有する（Gist は長いテキストや複数ファイルの共有に適しています）。
   - PR にファイル添付（Attach files by dragging & dropping, or use GitHub UI to upload）して、コメント内では添付ファイルの位置と目的を明示する。
   - VS Code の Copilot チャットやターミナル経由でコマンドやスクリプトを直接貼ると UI による改行や解釈で失敗することがあるため、スクリプトやパッチはローカルに一時ファイルとして保存してから添付・共有する方法を推奨します（作業完了後は一時ファイルを削除してください）。
   - コメント内に短いコードスニペットを貼る場合は必ずコードブロックで言語指定を付け、長いコードはファイルや Gist を参照するようにしてください。
   - これらの共有方法を使うことで、GitHub UI や Copilot チャットでの描画崩れやエスケープの問題を防げます。
