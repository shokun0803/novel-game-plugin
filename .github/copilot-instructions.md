...
9. 大量のコード、またはファイル全体を提供する場合はコードブロック（```` ```php ... ``` ````）を使用し、HTML タグが直接解釈されて消えないように注意してください。
10. コメント本文に長いコードを埋め込むのではなく、必要に応じて Gist や PR 添付ファイル、またはローカルで一時ファイルを作成して添付する方法も使用できます（ローカルに一時ファイルを作成した場合は、作業後に削除してください）。
