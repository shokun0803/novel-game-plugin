...
- [ ] コメント主導レビューポリシー（anchor コメント → reply → 返信コミットの範囲でレビュー）が遵守されているか（PR の差分ではなく、コメント・返信に紐づくコミットのみを対象にレビューしたか）
 - [ ] PR コメント投稿ルール（`@copilot` 指示・コードブロックの扱い・エスケープ・過去コメントの参照等）が守られているか
