...
### PRコメント投稿ルール（レビュー依頼時の記載方針）
- PR のコメントで `@copilot` を利用する際は、`copilot-instructions.md` の "PRコメント投稿ルール" に従ってください。
- コメント冒頭で `@copilot` を明示し、問題箇所のファイル名・関数名・行の目安を記載してください。
- コメント内のコードは必ずコードブロックで囲み、`$` のような特殊文字や HTML タグの取り扱いに注意してください。
- 過去のPR コメントやコミットを確認し、手戻りのないように修正指示を作成してください。
