...
- `novel-game-plugin-sample-images-vX.Y.Z-part01.zip` - サンプル画像パッケージ（パート1、存在する場合）
- `novel-game-plugin-sample-images-vX.Y.Z-part02.zip` - サンプル画像パッケージ（パート2、存在する場合）
- `novel-game-plugin-sample-images-vX.Y.Z-part0N.zip` - サンプル画像パッケージ（パートN、存在する場合）
- 各サンプル画像 ZIP に対応する `.sha256` チェックサムファイル（存在する場合）
- `novel-game-plugin-sample-images-vX.Y.Z-all.zip` - サンプル画像まとめパッケージ（`assets/sample-images/` に元データがある場合）
- サンプル画像まとめ ZIP の `.sha256` チェックサムファイル（存在する場合）

#### サンプル画像パッケージについて

サンプル画像はプラグイン配布 ZIP には含めず、リポジトリ内の `assets/sample-images/` を元データとして別アセット化し、GitHub Release に添付します。自動リリースワークフローでは `scripts/build-sample-images.sh TAG --all` を実行し、元データが存在する場合に分割 ZIP とまとめ ZIP を生成します。これにより、プラグイン本体の ZIP サイズを抑えたまま、必要時のみ画像を取得できます。
...
**プラグイン側での利用**:
プラグインは有効化直後に自動ダウンロードは行わず、管理画面の「マイゲーム」画面でサンプル画像が未配置のときに表示される UI から取得を開始します。現行実装では Release アセットを参照し、`partNN.zip` が 2 件以上ある場合は分割 ZIP 群を優先し、そうでなければ `-all.zip`、さらに後方互換として単一 ZIP にフォールバックします。複数 ZIP は順次ダウンロード・検証・展開されますが、実行中にいずれかのアセット処理が失敗した場合は全体のダウンロード状態を `failed` として扱います。

**手動ダウンロードの場合**:
利用者が手動で取得する場合は `sample-images-{version}-all.zip` を優先し、`-all.zip` が無いリリースでは対象の `partNN.zip` をすべて取得して展開します。
...
6. **Release の確認**
   ...
   - プラグイン ZIP と SHA256 チェックサムがアセットとして添付されていることを確認
   - `assets/sample-images/` に元データがあるリリースでは、サンプル画像用の分割 ZIP、`-all.zip`、各 `.sha256` も添付されていることを確認
