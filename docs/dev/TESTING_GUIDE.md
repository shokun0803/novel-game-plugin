...
**前提条件:**
- リリースに `partNN.zip` 形式の複数サンプル画像 ZIP が存在する
- 可能なら同一リリースに `-all.zip` も存在する

**手順:**
1. ダウンロードを開始
2. 各アセットが順次ダウンロードされることを確認
3. `partNN.zip` が優先的に選択されていることを確認

**期待結果:**
- 全アセットが順次ダウンロードされる
- 進捗バーが全体の進捗を反映
- `partNN.zip` が 2 件以上ある場合は `-all.zip` より分割アセットが優先される
- 実行中にいずれかのアセット処理が失敗した場合、全体のステータスは失敗になる
- 失敗時はエラー詳細または失敗アセット情報が表示される
