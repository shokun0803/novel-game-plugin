...
- **ネスト構造（`_choices` の `text`/`flag` など）のキー単位差分に対応** — 連想配列の要素をキーごとに行分割することで、変更されたキーだけがコア標準の行差分でハイライトされ、未変更のキーは「変更なし」として区別されるようにした
- **表示制御オプションを追加** — リビジョン比較画面に「変更行のみ表示」「同一行の折りたたみ表示」「JSON形式で出力」の操作UIを追加した

### Added (機能追加) — リビジョン比較の配列フィールド詳細差分表示（段階2）

- **配列フィールドの要素単位の差分表示に対応** — `_dialogue_texts` / `_dialogue_backgrounds` / `_dialogue_speakers` / `_dialogue_flag_conditions` / `_choices` / `_scene_arrival_flags` について、配列の要素1つを1行として出力するようにし、WordPressコア標準の行単位差分（追加/削除/変更のハイライト、変更部分の単語単位の強調）がそのまま配列要素単位の差分として機能するようにした
- 要素数が多い配列（200件超）では個別列挙をスキップし、件数のみを表示するフォールバックにした

### Fixed (バグ修正)

- **カスタムフィールドの差分表示がリビジョン比較画面に一度も表示されていなかった不具合を修正** — WordPressコアが実際に呼び出すフィルタ名は `_wp_post_revision_field_{$field}`（先頭にアンダースコア）だが、登録していたフィルタ名には先頭のアンダースコアが欠落しており、v1.2.0以降、カスタムフィールドの差分表示機能そのものが一度も画面に反映されていなかった（常にコア標準の生JSON文字列比較にフォールバックしていた）。あわせて、コアはこのフィルタをリビジョンのfrom側・to側それぞれ個別に呼び出し、戻り値2つをコア自身の `wp_text_diff()` で突き合わせて差分表示する設計であることが判明したため、独自にAdd/Delete/Change等の差分HTMLを組み立てて返す実装から、各リビジョン単体の内容をプレーンテキストとして返しコアの行差分に委ねる実装に再設計した（従来方式のままフィルタ名だけ修正すると、コアが差分結果同士をさらに突き合わせてしまい、二重に差分化された表示になることを確認済み）
