...
- **サンプルゲーム** - 「マイゲーム」画面からワンクリックでインストール（学習用・オプトイン）
...
**📝 サンプルゲームのインストール（オプトイン）**
- サンプルゲーム **Shadow Detective（影の探偵）** は自動ではインストールされません。「マイゲーム」画面に表示される「サンプルゲームをインストール」ボタンから、明示的な操作でインストールします
...
A1. 「マイゲーム」画面の「サンプルゲームをインストール」ボタンから、任意でインストールできます（自動インストールは行われません）。削除後も同じボタンから再インストール可能です。
...
プレイヤーは探偵となり、実業家・黒崎誠の失踪事件を追います。関係者への聞き込みと証拠集めで
矛盾を暴き、終盤の3段階の推理フェーズで「黒幕・真相・監禁場所」を自ら結論づける、
推理体験を中心に据えた構成です。誤った推理には代償が伴います。

### ゲーム仕様
- **シーン数**: 25シーン（調査ハブ「事件簿」を中心としたハブ&スポーク構造）
- **エンディング**: 4種類（完全解決・部分解決・誤推理・袋小路）
- **証拠アイテム**: 5種類（懐中時計・佐藤の写真・黒崎の手記・金庫の鍵・裏帳簿）
- **証言・進捗フラグ**: 9種類 ＋ 推理の回答記録フラグ4種類（計18フラグ）

### 特徴
- **3段階の推理フェーズ**: 集めた証拠が正解選択肢を解錠。証拠不足なら結論を出せず、誤答は誤推理エンドへ
- **条件付き選択肢**: フラグ条件（true/false・AND）で調査先が動的に開閉。探索済みの行き先は自動的に消える
- **選択肢によるフラグ設定（setFlags）**: 推理の回答を記録
- **条件付きセリフ**: 証拠に応じて独白が変化（alternative）・不要な独白が消える（hidden）
- **表情差分・回想演出**: セリフ単位のキャラ画像切替、自白シーンでの背景切替（回想）
- **国際化対応**: すべてのテキストが翻訳可能（日本語訳同梱）

### 詳細ドキュメント
- [シナリオ詳細設計](docs/shadow-detective-scenario.md) - 全25シーンの構成・フラグ設計・分岐フロー

### プレイ方法
1. 「マイゲーム」画面の「サンプルゲームをインストール」ボタンからインストールします（オプトイン）
...
### インストール仕様
- **オプトインインストール**: 「マイゲーム」画面のボタンからユーザーの明示的な操作でのみ実行（v1.6.0以降、有効化時の自動インストールは廃止）
...
- **リビジョン比較の高度差分（段階3）**
  - 配列要素の並び替え（移動）検出に対応（削除+追加ではなく「移動」として注記）
  - `_choices` などの連想配列要素をキー単位で差分検出できるように改善
  - 「変更行のみ表示」「同一行の折りたたみ表示」「JSON形式で出力」の操作UIを追加
- **リビジョン比較の配列フィールド詳細差分表示（段階2）**
  - セリフ・背景・話者・選択肢・フラグ条件・シーン到達フラグの各配列を要素単位で差分表示
- **バグ修正**
  - カスタムフィールドの差分表示がリビジョン比較画面に一度も表示されていなかった不具合を修正（v1.2.0以来の既存バグ）
  - 選択肢（`_choices`）を含むシーンでリビジョン比較・復元が壊れる不具合を修正
  - 詳細は [CHANGELOG.md](CHANGELOG.md) を参照

### Version 1.6.1
- **スマートフォン表示の最適化**
  - 画面下端でセリフが見切れる問題を修正（モーダルの高さ基準を実表示領域=dvhに変更、セーフエリア対応）
  - 立ち絵が小さく宙に浮いて見える問題を修正（縦画面での最大幅を拡大し、セリフボックスに接地する配置に）
  - スマートフォン横向きがタブレット扱いになっていた問題を修正（判定を幅ベースから高さベースに変更）
  - 選択肢が画面外や閉じるボタンに食い込む可能性を解消
  - 横向きで広告バナーと閉じるボタンが重なる問題を修正
  - 詳細は [CHANGELOG.md](CHANGELOG.md) を参照

### Version 1.6.0
- **プレイ画面デザインの全面刷新（Midnight Noir デザインシステム）**
  - セリフボックス・選択肢・タイトル画面・エンディング画面・各種モーダルを金彩×ダークトーンで統一
  - セリフのタイプライター表示、背景切替のクロスフェード演出を追加
  - セリフ表示中に文字が上へせり上がる不具合、狭いボックス前提の固定文字数改行による余白の不具合を修正
- **サンプルゲーム「影の探偵」のシナリオを全面改稿（v2 / 25シーン）**
  - 調査ハブ「事件簿」を中心としたハブ&スポーク構造に再構成
  - 証拠が正解選択肢を解錠する3段階の推理フェーズ（黒幕・真相・監禁場所）を追加
  - エンディングを4種類（完全解決・部分解決・誤推理・袋小路）に拡張
  - 登場人物・背景画像は既存アセットを維持
- **広告バナーのプレビューモード追加**
  - 広告プロバイダー未設定の環境でも `?nvl_ad_preview=1` またはコンソールの
    `novelGameToggleAdPreview(true)` でダミー広告バナーを表示し、レイアウトを確認可能に
- **バグ修正**
  - サンプル生成時に `_dialogue_flag_conditions` がJSON文字列で保存され、条件付きセリフが
    機能していなかった問題を修正
