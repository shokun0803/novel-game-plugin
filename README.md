...
6. **互換性設定（オプション）**
   - 管理画面「ノベルゲーム」→「ゲーム設定」→「互換性設定」
   - 「条件で内容変更」モードの挙動を選択
     - **新挙動（推奨）**: 条件成立時に代替テキストを表示
     - **旧挙動**: 条件成立時に通常テキストを表示（既存インストールのデフォルト）

### 条件付きセリフ表示機能

#### 表示制御モード
1. **通常表示** - フラグ条件に関わらず常に表示
...
**注意**: 既存のインストールは互換モードが自動的に有効になり、旧挙動を維持します。新規インストールでは新挙動がデフォルトです。
...
### Version 1.2.0 (予定)
- 「条件で内容変更」モードの仕様見直し
  - 新挙動: 条件成立時に代替テキストを表示、条件不成立時に通常テキストを表示
  - 旧挙動との互換性を保つため、互換モード設定を追加
  - 既存インストールは自動的に互換モード（旧挙動）を維持
  - 新規インストールは新挙動がデフォルト
- 管理画面のUI文言を直感的に改善
  - 「条件で非表示」→「条件成立時に非表示」
  - 「条件で内容変更」→「条件成立時に内容変更（代替テキスト表示）」
- 代替テキスト未入力時のフォールバック処理を追加（空表示を回避）
