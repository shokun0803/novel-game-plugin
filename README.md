...
- **ゲーム・シーン削除** - 不要なゲーム・シーンをごみ箱へ移動（復元可能）
...
- **ゲーム削除** - ゲームを削除（シーンはごみ箱へ移動、ゲーム設定は完全削除）
...
ゲームやシーンを個別に削除できます。削除したアイテムはWordPressのごみ箱に移動し、復元が可能です。
...
**📝 削除時の動作**:
- ゲームに含まれるすべてのシーンは**ごみ箱へ移動**されます（復元可能）
- ゲーム設定（フラグマスタなど）は**完全削除**されます（復元不可）
- シーンは WordPress の投稿一覧のごみ箱から復元できます
...
**⚠️ 重要**: ゲーム削除時はフラグマスタやゲーム基本設定は完全削除されますが、シーンはごみ箱に残ります。シーンを復元した場合、ゲーム設定を再作成する必要がある場合があります。
